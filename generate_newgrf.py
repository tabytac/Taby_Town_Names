...
GITHHUB_COUNTRY_DEMONYM_URL = "https://raw.githubusercontent.com/mledoze/countries/master/dist/countries.csv"
...
def get_country_demonym(country_code):
    ...
    with open(os.path.join(DATA_PATH, "countries.csv"), 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        for columns in reader:
            if country_code == columns[DEMONYM_COUNTRY_CODE_COLUMN].strip('"').strip("'"):
                return columns[DEMONYM_COUNTRY_DEMONYM_COLUMN].strip('"').strip("'")
...
def prepare_output_dir(output_dir, country_code, region_code, subregion_code):
    ...
    if not os.path.exists(os.path.join(BASE_PATH, "Source_Files", "license.txt")):
        print("Downloading license file")
        subprocess.run(["curl", "-o", os.path.join(BASE_PATH, "Source_Files", "license.txt"), NEWGRF_LICENSE_URL], check=True)
    if not os.path.exists(os.path.join(output_dir, 'license.txt')):
        print("Copying license file")
        subprocess.run(["copy", os.path.join(BASE_PATH, "Source_Files", "license.txt"), os.path.join(output_dir, 'license.txt')], shell=True, check=True)
...
def update_language_file(output_dir, country_code, region_code, subregion_code, version, num_towns, lowest_population):
    ...
    if subregion_name != "":
        country_demonym_a = f"{country_demonym_a} {region_name} {subregion_name}".strip()
...
def determine_input_data(country_code):
    ...
    if MERGED_FILE_OVERIDE or country_code == "":
        if not os.path.exists(os.path.join(DATA_PATH, "allCountries.txt")):
            download_data_files("allCountries.zip", DATA_URL + "allCountries.zip")
